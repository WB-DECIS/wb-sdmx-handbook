...
| **Context**                                   | The kind of artefact the key is resolved against: `datastructure`, `dataflow` or `provisionagreement`; `dataflow` is the normal choice | `/dataflow`                                                                                                                                                           |
...
| **Filter**                                    | The series key: one value per dimension of the DSD, in DSD order (`LOCATION.TRANSACT.MEASURE`)                        | `AUS.P51.VOB` → Australia (AUS), transaction P51 (Gross fixed capital formation), measure VOB (Constant prices, OECD base year)                                            |
...
Each part of the URL path dives deeper into the SDMX model (service → context → agency → dataflow → version → key), and the query string refines what slice of metadata you want returned and in which format. Note that nothing in the path says "metadata": the same path returns observations. What turns this into a referential-metadata query is the pair `attributes=msd&measures=none`. The full grammar — every path segment, the series key and its operators, the query parameters and the `Accept` header — is covered in the chapter *Working with the SDMX REST API* later in this handbook.
